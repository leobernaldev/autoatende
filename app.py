...
- Avaliações TOTALMENTE GRATUITAS
...
def chat():
    ...
    if sessao_id not in conversas:
        conversas[sessao_id] = []
    conversas[sessao_id].append({"role": "user", "content": mensagem})
    ...
    if "AGENDAMENTO_CONFIRMADO" in resposta_texto:
        try:
            ...
            pass
    conversas[sessao_id].append({"role": "assistant", "content": resposta_texto})
    return jsonify({"resposta": resposta_texto, "agendamento": agendamento})
